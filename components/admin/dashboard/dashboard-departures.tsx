...
import type { DashboardDeparture } from "@/lib/queries";
import {
  channelKeyFromSource,
  paymentBucket,
} from "@/components/admin/calendar/types";

interface DashboardDeparturesProps {
...
/**
 * Today's check-out list — Server Component. Same shape as the arrivals
 * panel but keyed on `checkOut`.
 */
export function DashboardDepartures({ departures }: DashboardDeparturesProps) {
  return (
    <section className="dashboard-panel">
      <header className="dashboard-panel-head">
        ...
        <Link
          href="/admin/reservations?filter=checkout_today"
      ...
      {departures.length === 0 ? (
        ...
        <ul className="dashboard-arrival-list">
          {departures.map((r) => {
            ...
            const checkOutLabel = formatInTimeZone(r.checkOut, TZ, "HH:mm");
            return (
              <li key={r.id} className="dashboard-arrival">
                ...
                >
                  ...
                  <div className="dashboard-arrival-side">
                    <div className="dashboard-arrival-time">
                      {checkOutLabel}
                    </div>
