...
import { ListingPillFilters } from "@/components/public/listings/listing-pill-filters";
...
const PILL_OPTIONS = [
  { value: "", label: "Tous" },
  { value: "pool", label: "Piscine privée" },
  { value: "terrace", label: "Avec terrasse" },
  { value: "duplex", label: "Duplex" },
  { value: "pets", label: "Animaux acceptés" },
  { value: "ac", label: "Climatisation" },
  { value: "flexible", label: "Annulation flexible" },
  { value: "padel", label: "Padel inclus" },
];
...
export default async function BungalowsListingPage({
  ...
  const minCapacity =
    guestsParam && capacityParam
      ...
      : (guestsParam ?? capacityParam);
  const activeTags = sp.tags ? sp.tags.split(",").filter(Boolean) : [];
  const hasPrivatePool =
    sp.pool === "1" || activeTags.includes("pool") ? true : undefined;
